...
class SNGraphDataset(GraphDataset):
    ...
    def __getitem__(self, i):
        ...
        while ntries < self.max_tries and len(negs) < nnegs:
            ...
            if n not in self._weights[int(t)]:
                negs.add(n)
            ntries += 1
        if len(negs) == 0:
            negs.add(t)
        ...
        return th.LongTensor(ix).view(1, len(ix)), th.zeros(1).long()  # TODO : SB  LongTensor
